        ...
        <div className="flex flex-col gap-3">
          {pendingIncoming > 0 && (
            <div className="p-3 border rounded-lg">
              <p className="text-sm">
                Bạn có <span className="font-bold text-primary-500">{pendingIncoming}</span> yêu cầu đổi card đang chờ xử lý.
              </p>
            </div>
          )}
          {recentUpdates.length > 0 && (
            <div className="p-3 border rounded-lg">
              <p className="text-sm font-semibold mb-2">Cập nhật yêu cầu đã gửi:</p>
                ...
                {recentUpdates.map((req: any) => {
                  ...
                  return (
                    <div key={req._id} className="flex items-center justify-between text-sm">
                      ...
                      {req.status === "accepted" ? (
                        <Badge className="bg-green-500 text-white text-xs shrink-0 ml-2">Đã chấp nhận</Badge>
                      ) : (
                        <Badge variant="secondary" className="text-xs shrink-0 ml-2">Đã từ chối</Badge>
                      )}
                    </div>
                  );
                })}
