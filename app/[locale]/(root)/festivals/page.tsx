    ...
    <div className="wrapper my-8 flex flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <Button asChild className="button">
          <Link href="/festivals/create">{t('createNew')}</Link>
        </Button>
      </div>

      {!festivals || festivals.length === 0 ? (
        <div className="flex-center min-h-[200px] w-full flex-col gap-3 rounded-[14px] bg-grey-50 dark:bg-muted py-28 text-center">
          <h3 className="p-bold-20 md:h5-bold">{tf('empty')}</h3>
          <p className="p-regular-14">{tf('emptyHint')}</p>
        </div>
      ) : (
            ...
            >
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full ${
                    ...
                    {f.isActive ? tf('statusActive') : tf('statusInactive')}
                  </span>
                </div>
              ...
              <div className="flex gap-2 shrink-0">
                <Button size="sm" variant="outline" asChild>
                  <Link href={`/festivals/${f._id}/update`}>{t('edit')}</Link>
                </Button>
