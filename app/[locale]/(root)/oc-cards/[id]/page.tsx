...
export default async function OcCardDetail(props: OcCardDetailProps) {
  ...
  const t = await getTranslations('ocCard');
  if (!card) {
    return (
      <div className="wrapper my-8 text-center">
        <h2 className="h2-bold">{t('notFound')}</h2>
      </div>
    );
  }
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-muted-foreground">{t('belongsTo', { name: card.ownerName })}</p>
                {card.images?.[0]?.artistName && (
                  <p className="text-sm text-muted-foreground">{t('artistLabel')} {card.images[0].artistName}</p>
                )}
              </div>
              {isOwner && (
            ...
            <div className="flex gap-2 items-center flex-wrap">
              ...
              <span className="text-sm text-muted-foreground">{t('wantTradeCount', { count: tradeCount.total })}</span>
              {isOwner && (
            ...
            {(card.appearance?.text || card.appearance?.imageUrl) && (
              <div className="border rounded-lg p-4">
                <h3 className="font-semibold mb-2 flex items-center gap-2">
                  <User className="w-4 h-4" /> {t('identifyTitle')}
                </h3>
...
async function TradeRequestsList({ cardId, userId }: { cardId: string; userId: string }) {
  ...
  return (
    <div className="border rounded-lg p-4">
      <h3 className="font-semibold mb-3">{t('tradeListPageTitle', { count: requests.length })}</h3>
...
async function TradeRequestItem({ request, userId }: { request: any; userId: string }) {
  const t = await getTranslations('ocCard');
  ...
  return (
    <div className="flex flex-col gap-2 p-3 border rounded-md bg-grey-50 dark:bg-gray-800">
      ...
      {request.contactMethod && (
        <p className="text-sm text-muted-foreground">{t('contactMethod')}: <span className="font-medium text-foreground">{request.contactMethod}</span></p>
      )}
      {request.linkedCard && (
        <Link
        ...
        >
          🔗 {t('viewLinkedCard', { name: request.linkedCard.ownerName })}
        </Link>
      )}
    </div>
  );
